...
Selector.prototype = {
    ...
    _bindEvents: function(){
        ...
        jquery.delegate(".yselector_input", "click" + SELECTOR_EVENT, toggleEvent)
            ...
            .delegate(".yselector_suggest", "mouseenter" + SELECTOR_EVENT, function(e){
    ...
    _drawHtml: function(){
        ...
        var fullHTML = ['<div class="yselector">',
                            ...
                            '</div>',
	                        '<div class="yselector_suggestwrap">',
    ...
    _drawSuggest: function(){
        ...
        for (var i = 0, l = list.length; i < l; i++) {
            item = list[i];
            listHtmlArray.push('<li><a data-value="' + item.value + '" title="'+ item.text +'" hidefocus="on" data-index="' + i + '"');
    ...
    _setOptions: function(obj){
        ...
        $(rawSelect).change(function(){
            self.index($(this).get(0).selectedIndex);
        });
    ...
    _show: function(){
        ...
        var top = direction === "top" ? 0 - height - self.option("jquery").height() : -1;

        suggest.css("height", height).css("top", top + "px");
    },
    _hide: function(){
...
QNR.timeCount = function(t,o,timeout){
  var prt = o.parents('.popup_tips_wrap');
  this.timeoutNum = '';
  this.o = o;
  this.prt = prt.length > 0 ? prt : o.parents('.time_count_ctn');
  this.t = t;
...
QNR.timeCount.prototype = {
  ...
  count: function(noJump){
    ...
    var secs = 1000;
    if(_self.dealTime(t)){
      dur.days = Math.floor(t / days);
      dur.hours = Math.floor((t - dur.days * days) / hours);
      dur.mins =  Math.floor((t - dur.days * days - dur.hours * hours) / mins);
      dur.secs = Math.floor((t - dur.days * days - dur.hours * hours - dur.mins * mins) / secs);
  ...
  show: function(dur){
    var o = this.o;
    if(dur.days > 0){
      o.html(dur.days + '天' + dur.hours + '时' + dur.mins + '分' + dur.secs + '秒');
    }else if(dur.hours > 0){
      o.html(dur.hours + '时' + dur.mins + '分' + dur.secs + '秒');
    }else if(dur.mins > 0){
      o.html(dur.mins + '分' + dur.secs + '秒');
    }else{
      o.html(dur.secs + '秒');
    }
  }
 ...
 *	对异步请求中的session超时处理
 *
 *	调用范围	所有有异步请求的支付srclist文件
 *
 *	逻辑范围	检查session超时 {ret: false, errcode: -1}, 弹出登录或者转入登录
 *
 *	标志位		无
 *
 *	@Requires	-jquery
 *				-class/cross-domain.js
 *
 *	@notice		检查session超时的规则为errcode: -1
 *				对超时之后有两个处理, 第一, 页面声明了timeout-form，这样就弹出登录框，登录成功则将timeout-form提交; 第二, 直接刷新页面
 *
 **/
...
var quickPayTemplate = {
	...
	icon_msg: ['<span class="icon_tips_wrap"><em class="icon_tips imsg"></em><em class="txt_tips">{content}</em></span>'],
	
	icon_tips: ['<span class="icon_tips_wrap {type} {cls}"><em class="icon_tips"></em><em class="txt_tips">{content}</em></span>'],
	...
	creditCardDialog : ['<div class="b_fpanel bank_width b_chose_bank">',
									'<div class="container">',
										'<div class="inner inner1 b_tab_ui">',
											'<div class="e_title clrfix"><a id="banllist_close" href="javascript:void(0);" class="close">关闭</a><span class="title_txt">信用卡网上银行</span></div>',
											'<div class="content_wrap b_bank_list debit_card debit_card_ctn">',
											'</div>',
										'</div>',
									'</div>',
								'</div>'],
	creditUCardDialog : ['<div class="b_fpanel b_other_bank b_pinfo_form popup_window_other_card">',
									'<div class="container">',
										'<div class="inner">',
										    '<div class="e_title clrfix"><span class="title_txt">使用其他信用卡</span><a href="javascript:void(0)" class="close">关闭</a></div>',
											'<div class="content_wrap"><form></form></div>',
										'</div>',
									'</div>',
								'</div>'],
	creditUCardDialog1 : ['<div class="b_fpanel b_other_bank b_pinfo_form popup_window_other_card">',
									'<div class="container">',
										'<div class="inner">',
										    '<div class="e_title clrfix"><span class="title_txt">使用其他信用卡</span><a href="javascript:void(0)" class="close">关闭</a></div>',
									...
									'</div>',
								'</div>'],
	confirmCardInfo: ['<div class="b_fpanel restip b_pinfo_form">',
						'<div class="container b_write_card">',
							'<div class="inner e_write_inner">',
								...
								'<div class="content_wrap">',
									'<div class="confirmCardInfo"><span class="t1"></span><span>与该银行绑定的姓名或身份证号可能有误，请再次输入</span></div>',
									'<div class="credit-bank-wrap"><div class="fL">{img}</div><div class="fR">信用卡 ** <span>{tel}</span></div></div>',
									'<table width="100%" cellspacing="0" cellpadding="0" class="ftable">',
										'<tr class="js_credit_tr">',
											'<td class="c1">姓名：</td>',
											'<td>',
												'<div class="ops_cardnum">',
													'<div class="set input confirm">',
														'<input type="text" value="" placeholder="输入姓名需与注册银行卡相同" class="grey_txt txtbox" data-jvalidator-pattern="not_empty" name="cardholder">',
													'</div>',
										...
										'<tr class="js_credit_tr">',
											...
											'<td>',
												'<div class="ops_cardnum">',
													'<div class="set input idsuggest-wrap confirm"><input type="text" value="" placeholder="请输入与银行卡绑定的证件号" class="grey_txt txtbox" data-jvalidator-pattern="not_empty & idcard" name="identityCode"></div>',
													'<div class="set popup_tips_wrap js_tipcontainer"></div>',
												'</div>',
											'</td>',
										'</tr>',
										'<tr>',
											'<td></td>',
											'<td><a href="javascript:void(0)" class="res-btn" id="re_confirm">确认并支付</a></td>',
										'</tr>',
									'</table>',
								'</div>',
							'</div>',
	...
	payDoneDialog : ['<div class="b_fpanel b_chose_bank ">',
									'<div class="container payResult">',
										'<div class="inner">',
											'<div class="e_title clrfix"><span class="title_txt">支付结果</span></div>',
											'<div class="content_wrap">',
												'<div class="info hide"></div>',
												'<div class="ctl"><a href="#" id="paySuccess">支付成功</a><a href="#" id="payFail">支付失败，继续支付</a></div>',
												'<div class="msg">',
													'<p class="title"><span class="t1"></span><span class="t2">问题解答</span></p>',
													'<p class="ctn"><span class="head">1. 网上银行页面打不开，或者打开无法支付，怎么办？ </span><a class="show" href="#"><span>查看</span><span class="down"></span></a></p>',
													'<p class="more hide">答：建议使用IE浏览器，若您当前使用非IE浏览器，建议使用信用卡支付方式支付订单。</span>',
													'<p class="ctn"><span class="head">2. 总是支付失败，提示银行卡限额不足怎么办？ </span><a class="show" href="#"><span>查看</span><span class="down"></span></a></p>',
													'<p class="more hide">答：银行卡限额是指某银行对该行银行卡单笔消费额度的限制，如果您的银行卡限额不足，',
													'您可以充值去哪儿网余额账户，充值足额后再使用余额支付来完成交易。</p>',
													'<p class="ctn"><span class="head">3. 我在网上银行重复支付多次怎么办？ </span><a class="show" href="#"><span>查看</span><span class="down"></span></a></p>',
													'<p class="more hide">答：请您立刻拨打去哪儿网客服核实重复支付情况，一经核实后；去哪儿网会将多支付款项退款至您的原支付银行卡中。</p>',
												'</div>',
											'</div>',
										'</div>',
									'</div>',
...
(function(){
	...
	var mask=function(){
		...
		_m.css({'position':'absolute', 'z-index':'3000', 'opacity':0.5, 'top':0, 'left':0, 'width':"100%", 'background':'black'});
...
(function($){
  ...
  var tips = QNR.locale.tips;
  var showAllBindBank = $('#showAllBindBank');
  var expires_year = $(".expires_year");
  var expires_month = $(".expires_month");
  var quickPayForm = $("#quickpayform");
  var reloadForm = $('#reload-form');
  ...
  var cardPicWrapAllCard = $('#allcardsCon .e_card_picwrap');
  var cardPicWrapBindCard = $('#bindcardsCon .e_card_picwrap');
  var creditInfoWrap = $(".credit-info-wrap");
  var imageURL = '/images/site/images/pay/bankicon_1/';
  //倒计时
  var ttime = $("#timeCount");
  //选择卡类型
  var cardType = $("input:radio[name='cardType']");

  function unionPay(){
    this.netPayUrl = baseurl + 'page/union/netbankPay.do?';
    ...
    this.getPayResultUrl = baseurl + 'page/cashier/netbankResult.do';
    this.getUserId = baseurl + 'page/cashier/queryIdentity.do',
    this.getBankInfoUrl = baseurl + 'page/cashier/getBankInfoByCard.do';
    this.getBankInfoByCardUrl = baseurl + 'page/cashier/getBankInfoByCard.do';
    this.detail_url = baseurl + 'page/cashier/queryProductDetail.do';  //获取订单详情
    this.mobileStage = bindcardsCon.find('input[name="mobile"]');
    this.cardInfoData = {};
    this.isRequestCardInfo = {};
    this.timeCountObj = {};
    this.init();
  }
  unionPay.prototype = {
    ...
    initDebitCard: function(){
      ...
      //init 
      if(me.isBindCardShown()){
        var radioEle = bankLi.find('input:checked');
        me.securityControl(radioEle.parent().data('ctl'), radioEle);
      }
    },

    initBankCode: function(){
      //如果是用户已经保存的卡，那么把选择的卡code放到的银行卡code里；
      if(this.isBindCardShown()){
        this.currentMobile = radios.filter(':checked').attr('mobile')||'';
        this.mobileStage.val(this.currentMobile);
      }else{
        allcardsCon.show();
      }
    },
    ...
    getFormParams: function(){
      var array = reloadForm.serializeArray();
      var info = ["busiTypeId", "merchantCode","orderNo","orderDate","HMAC","UHMAC"];
      var arr = {};
      var arrKV = [];
      ...
      this.formParams = arr;
      this.formParamsKV = arrKV;
    },

    initUserIdCard: function(){
      if(QNR.PayInfo.userid != ''){
        var data = {userId: QNR.PayInfo.userid}, me = this;
        $.extend(data, me.formParams);
        data.HMAC = data.UHMAC;
        data.UHMAC = undefined;
        $.ajax({
          url: me.getUserId,
          data: data,
          type: 'POST',
          ...
          success: function(e){
            if(e && e.ret && e.data && e.data.length > 0){
              var fmtData = [];
              $.each(e.data, function(idx, v){
                smpV = v.replace(v.substr(6, 8), '&nbsp;&nbsp;********&nbsp;&nbsp;');
                fmtData.push({id: idx, smp: smpV, real: v});
              })
              var idIpt = allcardsCon.find('input[name=identityCode]');
              idIpt.autocomplete(fmtData,{
                minChars: 0,
                max: 5,
                width: 220,
                matchContains: false,
    ...
    initPlaceHolder: function(){
      var allIpt = $('#allcardsCon table input[name], #bindcardsCon table input[name]');
      var blurAction = function(ele){
        if($.browser.msie){
          if($.trim(ele.val()) != ''){
            ele.css('font-weight','bold').css('color','#333');
            if(ele.is('#credit_cardnum')){
              ele.css('font-size', '16px');
            }
          }else{
            ele.css('font-weight','normal').css('color','#A9A9A9').css('font-size', '12px');
          }
        }
      };

      var focusAction = function(ele){
        ele.css('font-weight','bold').css('color','#333');
        if(ele.is('#credit_cardnum')){
          ele.css('font-size', '16px');
        }
      ...
      allIpt.bind('focus blur', function(e){
        var $this = $(this);
        if(e.type == 'focus'){
          focusAction($this);
        }else if(e.type == 'blur'){
    ...
    initEvent : function(){
      var me = this;
      //酒店的说明：
      $('#information_ctl').mouseenter(function(e){
        $('#information_ctn').show();
      }).mouseleave(function(e){
        $('#information_ctn').hide();
      });
      ...
      $('#allcardsCon table input[name], #bindcardsCon table input[name]').bind('keyup change', function(){
        var $this = $(this); text = $this.val();
        if(text){
          if($this.is('#credit_cardnum, #debit_cardnum')){
            $this.val(me.formatCardNumber(text));
          }else if($this.is('#mobile,#bindcardsCon #mobile')){
            $this.val(me.formatMobileNumber(text));
          }else if($this.is('[name="cardholder"]')){//输入汉字时不进行时时去空格，在jvalidater时进行去空格
          }else{
            $this.val(me.formatIpt(text));
          }
        }
      });

      $('#allcardsCon table input[name], #bindcardsCon table input[name]').bind('focus', function(e){
        var $this = $(this);
        var prtTable = $this.parents('table');
        var siblingCardInfo = prtTable.siblings('.credit-info-wrap');
        var siblingImg = prtTable.siblings('.e_card_picwrap');
        if($this.is('[name="cvv"]')){
          siblingCardInfo.hide();
          siblingImg.show().find('.card_pic .img').css('background-position', '-176px 0');
        }else{
          siblingCardInfo.find('.credit-info').children().length ? siblingCardInfo.show() : siblingCardInfo.hide();
          siblingImg.hide();
        }
      });

      $('#allcardsCon .yselector_box, #bindcardsCon .yselector_box').live('click', function(e) {
        var $this = $(this);
        var prtTable = $this.parents('table');
        var siblingCardInfo = prtTable.siblings('.credit-info-wrap');
        var siblingImg = prtTable.siblings('.e_card_picwrap');
        ...
        creditInfoWrap.hide();
        siblingImg.show().find('.card_pic .img').css('background-position', '0 0');
      });
      ...
      /*$(".idsuggest-wrap input").live("keyup",function(){
        debugger;
        if(true)
        $(this).siblings(".idsuggest").remove().end().after('<ul class="idsuggest"> <li><a href="javascript:void(0)">6225 ********* 3529</a></li> <li><a href="javascript:void(0)">6225 ********* 3529</a></li> </ul>')
      }).live("blur",function(){
      ...
      bindcardsCon.delegate('li.js-choose-item', 'mouseenter',function(e){
        ...
        var el = $(this).closest('li'), radioEle = el.find('input[type=radio]');
        var ctl = $(this).data("ctl");

        var updateBindCardEle = function(ele){
          //顺序不能错，要先清除输入痕迹，再填充手机号；
          me.clearAllTips(bindcardsCon.find('table'));
        ...
        // 风险控制
        !me.isRequestCardInfo[radioEle.data('id')] && me.securityControl(ctl, radioEle, updateBindCardEle);
      });
      ...
      $('#btnSubmitDebitCard_next, #btnSubmitPlatform_next').click(function(){
        var $this = $(this);
        //点击下一步跳转到银行页面，倒计时结束时不跳转；
        me.timeCountObj && me.timeCountObj.suspend();
      ...
      $('.detail_more_ctl .show').click(function(){
        var $this = $(this), spans = $this.find('span');
        $this.parents('.detail_more').find('.pay_count').toggle();
        $('#pay_count_detail').toggle();
        if(spans.eq(1).is('.down_1')){
          spans.eq(1).removeClass('down_1').addClass('up_1');
          $this.parent().next('.detail_more_ctn').show().parent().addClass('bottom_border');

          //向后端发送请求，获得订单详情信息
          if(!me.isRequestOrderDetails){
            $.ajax({
            url:me.detail_url,
            data: me.formParams,
            type:"POST",
            ...
            complete:function(res){
                if(res.responseText){
                  $('#detail_more_ctn_box').html(res.responseText);
                  me.isRequestOrderDetails = true;
                }else{
                  $('#detail_more_ctn_box').html("未获得详细信息");
                }                
            }})
          }          
        }else{
          spans.eq(1).removeClass('up_1').addClass('down_1');
          $this.parent().next('.detail_more_ctn').hide().parent().removeClass('bottom_border');
        }
        return false;
      });
    ...
    updateBindMobile: function(ele){
      var mb = ele.attr('mobile'), me = this;
      if(mb == ''){
        me.mobileStage.attr('readonly', false);
      }else{
        me.mobileStage.attr('readonly', true);
      }
      me.mobileStage.val(mb);
      me.currentMobile = mb;
    ...
    formatIpt: function(num){
      return num.replace(/\s/g, '');
    },

    updateCardInfoData: function(ele){
      var me = this;
      me.cardInfoData['bankCode'] = ele.attr('bankCode');
      me.cardInfoData['channel'] = ele.attr('channel');
      me.cardInfoData['tppCode'] = ele.attr('tppCode');
      me.cardInfoData['isDirect'] = ele.attr('isDirect');
      me.cardInfoData['inputItemHMAC'] = ele.attr('inputItemHMAC');
      //有数据时跟新一下手机信息；
      me.updateBindMobile(ele);
    },

    // 风险控制
    securityControl:function(ctl, radioEle, callback){
      var me = this;
      var securityData = {};
      var iptLi = radioEle.parents('li');

      if(!ctl){
        var cardId = radioEle.data('id');
        var data = {cardId: cardId, pmCode: 'QCARD'};
        $.extend(data, me.formParams);
        //判断上一个请求是否完成；避免重复请求；
        me.isRequestCardInfo[cardId] = true;
        $.ajax({
          url:me.getBankInfoByCardUrl,
          data: data,
          type: 'POST',
          dataType: 'JSON',
          success:function(res){
            if(res.ret && res.data && res.data.length > 0){
              ...
              securityData = resData.inputItem;

              radioEle.parent().data('ctl', resData.inputItem);
              radioEle.attr('bankCode', resData.bankCode);
              radioEle.attr('channel', resData.channel);
              radioEle.attr('isDirect', resData.isDirect);
              radioEle.attr('tppCode', resData.tppCode);
              radioEle.attr('mobile', resData.mobile);
              radioEle.attr('inputItemHMAC', resData.inputItemHMAC);
              me.updateCardInfoData(radioEle);

              //添加限额
              iptLi.addClass("current");
              iptLi.find('.min').text('小于' + resData.maxAmout + '元');
              iptLi.find('.minDay').text('小于' + resData.maxDayAmout + '元');

              callback && callback(radioEle);
            }else{
              //未查询到数据时，显示所有的输入框
              //TODO:这里到底是留个还是四个呢？默认六个吧；未确定方案
              //TODO: 未查询到结果时，bankCode/channel/isDirect/tppCode/mobile从哪里找数据？
              securityData = {identityCode:true,cvv:true,vCode:true,cardholder:true,validate:true,mobile:true};
            }
            me.securityControlHandler(securityData);
            me.isRequestCardInfo[cardId] = false;
          }
        });
      }else{
        this.securityControlHandler(ctl);
        me.updateCardInfoData(radioEle);
        callback && callback(radioEle);
      }

      me.updateBindMobile(radioEle);
    },

    parseJSONToString: function(json){
      var arr = [], temp;
    ...
    securityControlHandler:function( data ){
      // 风险控制内部函数
      var me = this;
      data = (typeof data == 'string' ? $.parseJSON(data) : data) || {};
      var $wrap = $(".ftable:visible");
      $.each(data, function(key,v){
        var $target = $wrap.find("tr#" + key);
        if($target.length){
          if(typeof v == 'string'){
            if(v == 'true'){
              $target.show();
      ...
      //当只有cvv2显示时，要一直显示右侧的提示信息；
      cardPicWrapBindCard.hide();
      if(!(data.identityCode || data.vCode || data.mobile || data.cardholder || data.validate)){
        if(data.cvv && me.isBindCardShown()){
          cardPicWrapBindCard.show().find('.card_pic .img').css('background-position', '-176px 0');
        }
      }
    },
    ...
    initTimeCount : function(){
      var me = this;
      var errorUrl = '/payServer/error.do?type=timeout';
      var ttime = $("#timeCount");
      ttime.parents('.popup_tips_wrap').hide();
      var formattime = function(t){
        t = t || '00000000000000';
        var res = t.match(/\d{2}/g), yy = res[0] + res[1], mou = res[2], dd = res[3];
        var hh = res[4], mm = res[5], ss = res[6];
        return (new Date(yy,mou,dd,hh,mm,ss)).getTime();
      };
      var newTimeEle = $('#newtime');
      var validTimeEle = $('#validtime');
      if(newTimeEle.length > 0 && validTimeEle.length > 0){
        var newtime = newTimeEle.val();
        var validtime = validTimeEle.val();
        var dur = formattime(validtime) - formattime(newtime);
        var timeout = function(){
          window.location.href = errorUrl;
        };

        me.timeCountObj = new QNR.timeCount(dur, ttime, timeout);
      }
    },
    ...
    submitform : function(e){
      ...
      this.formChecker.validateAll(function(ret) {
        ...
        if(ret && valiHidden){
          //暂停倒计时；
          me.timeCountObj && me.timeCountObj.pause();
          var data = me.getFormAllParams(quickPayForm);
          me.syncsubmit(quickPayForm, $.extend(data, me.cardInfoData), '_self');
        }
      });
      e.preventDefault();
    },

    syncsubmit: function(origin, params, tgt){
      var syncForm = document.createElement("form"), tgt = tgt || '_blank';
      document.body.appendChild(syncForm);
      syncForm.method = 'post';
      if(typeof origin == 'object'){// form object
        syncForm.action = origin.attr('action');
      }else{//url string
        syncForm.action = origin;
      }
      syncForm.target = tgt;
    ...
    //获取form表单
    getFormAllParams : function(form){
      var me = this;
      var ret ={}, mon, year;
      if(me.isBindCardShown() && allcardsCon.is(':visible')){
        form = $("#allcardsCon").parent();
        $.extend(ret, me.formParams);
      }else if(me.isBindCardShown()){
        allcardsCon.remove();
        bindcardsCon.find('.ftable tr').filter(':hidden').remove();
      }else{
        bindcardsCon.remove();
      }
      ...
      $.each(array,function(i,input){
        ...
        if(input.name == 'month' && !isNaN(parseInt(input.value, 10))){
          mon = input.value;
        }
        if(input.name == 'year' && !isNaN(parseInt(input.value, 10))){
          year = input.value;
        }
      });

      if(mon && year){
        ret['validate'] = year + mon;
      }else{
    ...
    resetPayStatus: function(){
      var me = this;
      if(!me.otherBankDlg) return ;
      me.clearAllTips(me.otherBankDlg.dom.find('table'));
      me.clearAllTips(bindcardsCon.find('table'));

      me.otherBankDlg.dom.find(".content_wrap form").children().appendTo(quickPayForm);
      allcardsCon.hide();
      cardPicWrapAllCard.hide();
      creditInfoWrap.hide();
      me.otherBankDlg.hide();
      $(".credit-bank").hide();
      creditInfoWrap.find('.credit-info').empty();
      me.otherBankDlg = null;
      bindcardsCon.find(".ftable").show();
      $(".b_other_bank").remove();
      me.isBindCardShown();

      //初始化常用卡支付中选中的radio标签中的数据；
      var radioEle = bankLi.find('input:checked');
      me.updateBindMobile(radioEle);
      me.updateCardInfoData(radioEle);
  },
    ...
    showOtherBankDialog:function(){
      var me = this;
      var obj = $("#allcardsCon,.b_pinfo_form .e_ops_button").show();

      var html = quickPayTemplate.creditUCardDialog.join('');
      me.otherBankDlg = new QNR.htmlDialog(html);
      me.otherBankDlg.show();
      $(".b_other_bank .content_wrap form").append( obj );
    ...
    showPayResultDialog: function(){
      ...
      this.payResultDlg.dom.find('.ctl a').click(function(){
        ...
        if($this.is('#paySuccess')){
          me.getPayResult(true);
        }else if($this.is('#payFail')){
          me.getPayResult(false);
        }
    ...
    getPayResult: function(uPress, func){
      var me = this;
      $.ajax({
        url: me.getPayResultUrl,
        type: 'POST',
        data: me.formParams,
        dataType: 'JSON',
        success: function(e){
          if(e.ret && e.retCode == '1'){//1成功
            var url = e.data[0].noticeUrl;
            me.syncsubmit(url, e.data[0], '_self');
          }else if(e.retCode == '0'){//0 校验不过
            alert('校验不过');
          }else if(e.retCode == '2'){//2失败
            //继续倒计时；如果倒计时已经结束，那么此刻会跳转页面；
            me.timeCountObj && me.timeCountObj.start();

            me.payResultDlg.hide();
            me.payResultDlg.remove();
            me.payResultDlg = null;
          }else if(e.retCode == '3'){//3未知
            if(uPress){
              me.payResultDlg.dom.find('.info').html('<span>抱歉当前未收到银行支付确认，为避免重复支付，请确认您的银行卡是否已经扣款。如已扣款请联系去哪儿网客服 10101234 进行处理。</span>').show();
              var temp = '<a href="#" id="payResultConfirm">我知道了</a>';
              me.payResultDlg.dom.find('.ctl a').unbind('click');
              me.payResultDlg.dom.find('.ctl').empty().append(temp);
              me.payResultDlg.dom.find('#payResultConfirm').click(function(){
                //继续倒计时；如果倒计时已经结束，那么此刻会跳转页面；
                me.timeCountObj && me.timeCountObj.start();
          ...
          }else if(e.retCode == '4'){//4异常
            me.payResultDlg.dom.find('.info').html('<span>抱歉当前系统繁忙，为避免重复，请稍后点击“支付成功”或“支付失败，继续支付”按钮刷重新查看订单状态；也可拨打10101234联系去哪儿网客服进行查询处理。</span>').show();
          }else{
            alert('unknown error!');
          }
          func && func();
    ...
    showCreditCardDialog: function(){
      ...
      if(!me.creditCardDlg){
        ...
        me.creditCardDlg.dom.find('#btnCreditSubmit_next').click(function(e){
          //点击下一步跳转到银行页面，倒计时结束时不跳转；
          me.timeCountObj && me.timeCountObj.suspend();
          me.netbankPay(me.creditCardDlg.dom.find('form'));
          return false;
        })
    ...
    showCreditCardInfo: function(cardId, ele, callback){
      var me = this, lastId = cardId.substring(cardId.length - 4);
      var data = {cardId: cardId};
      $.extend(data, this.formParams);
      $.ajax({
        url:this.getBankInfoUrl,
        dataType: 'JSON',
        type: 'POST',
        data: data,
        success:function(res){
          if(res.ret){
            var resData = res.data[0];
            $(".credit-bank-wrap").html('<div class="fL"><img src="' + resData.picUrl + '"></div><div class="fR">信用卡 ** <span>'+lastId+'</span></div>');
            $(".credit-bank").slideDown();
            cardPicWrapAllCard.hide();
            $(".credit-info").html('<p>网上交易限额</p><table><tr class="title"><td>单笔限额(元)</td><td>单日限额(元)</td></tr><tr><td>小于' + resData.maxAmout + '元</td><td>小于' + resData.maxDayAmout + '元</td></tr></table>');
            creditInfoWrap.show();

            var subIpts = ele.siblings();
            subIpts.length && subIpts.remove();

            me.cardInfoData['channel'] = resData.channel;
            me.cardInfoData['isDirect'] = resData.isDirect;
            me.cardInfoData['tppCode'] = resData.tppCode;
            me.cardInfoData['bankCode'] = resData.bankCode;
            me.cardInfoData['inputItemHMAC'] = resData.inputItemHMAC;
            callback && callback(true);
          }else{
            callback && callback(false);
            if(res.errcode == 100023){
              me.showInfoTips(ele, res.errmsg || 'error');
            }else{
              callback && callback(false);
              me.showTips(ele, res.errmsg || 'error');
            }
            creditInfoWrap.hide();
            $(".credit-bank").hide();
          }
    ...
    netbankPay: function(fm, dlg){
      var me = this, params = [];
      var arr = fm.serializeArray();

      var pmcode = fm.find('input:checked').attr('pmcode');
      arr.push({name: 'pmCode',value:pmcode});
      $.merge(arr, this.formParamsKV);

      me.syncsubmit(fm, arr);
    ...
    //短信验证码
    initMobile : function(){
      var form = quickPayForm;
      var _error = form.find("[placeholder='get-permit-error']");
      var _message = form.find('[placeholder="get-permit-message"]');
      var _button = form.find('[placeholder="get-permit"]');
      var _showError=function(msg){
        _error.removeClass("popup_tips_wrap_clr");
        var _msg = quickPayTemplate.icon_wrong.join('');
        _error.html( _msg.replace(/\{content\}/g,  msg) ).show();
      ...
      QNR.phoneValid({
        ...
        callback: function(data){
          ...
          if(data.ret){
            ...
            _error.removeClass("popup_tips_wrap_clr");
            var _msg = quickPayTemplate.icon_wrong.join('');
            _error.html( _msg.replace(/\{content\}/g,  tips.vcode.sendFail) ).show();
            return false;
          }
        ...
        locker: function(obj){
          var _mobileObj = $(obj).parents('table').find('input#mobile');
          var _mobile = (_mobileObj.val()).replace(/\s/g, '');

          if(!_mobile){
            _mobileObj.trigger('blur');
          }
        ...
        paramRender: function(obj){
          var _self = this;
          var $moble = $(obj).parents('table').find('input#mobile');
    ...
    valiHiddens : function(){
      var me = this, res = true, m, y, dateNow = new Date();
      var y0 = dateNow.getFullYear().toString().match(/\d{2}/g)[1];
      var m0 = dateNow.getMonth() + 1;
      if(me.isBindCardShown() && allcardsCon.is(':visible')){
      ...
      if(m.siblings().is(':visible') && y.siblings().is(':visible')){
        if(m.get(0) && y.get(0) && !m.parents('tr').is(':hidden')){
    ...
    //TODO:查找用到该函数的地方是否需要更多验证；
    isBindCardShown : function(){
      //修改提交按钮的文案；
      var res = bindcardsCon && bindcardsCon.is(":visible");
      if(res){
        if(allcardsCon.is(':visible')){
          btnSubmit.text('同意协议并付款');
        }else{
          btnSubmit.text('确认付款');
        }
      }else{
    ...
    initExpiresDate : function(){
      ...
      var expire_M_Y = function(ori){
        ...
        if(!!ori.options.holder[0].selectedIndex){
          ori.options.input.css('color', '#333').css('font-weight','bold');
        }else{
          ori.options.input.css('color', '#A9A9A9').css('font-weight', 'normal')
        }
      }
    ...
    clearAllTips: function(ctn){
      var me = this;
      var eles = ctn.find('input[name], select[name]');
      $.each(eles, function(idx, v){
        var ele = $(v);
        ele.removeClass('error');
        tipCtn = ele.closest('td').find('.js_tipcontainer');
        if(tipCtn.length){
          if(ele.is('input')){
            //fixed ie6 bug
            ele.val(ele.attr('data-placeholder-value'));
            ele.attr('data-placeholder-active','true');
            me.initPlaceHolder();
          }else if(ele.is('select')){
            ele.get(0).selectedIndex = 0;
            ele.trigger('change'); //触发控件选项变化；
          }
          tipCtn.find('.txt_tips').text('');
          tipCtn.find('.iright').hide();
          tipCtn.find('.iwrong').hide();
          tipCtn.find('.imsg').hide();
        }
      })
    },
    ...
    showInfoTips: function(el, msg){
      var _div = el.closest('td').find(".js_tipcontainer");
      _div.html( quickPayTemplate.icon_msg.join('').replace(/\{content\}/g,  msg) ).show();
      _div.show();
    },
    ...
    initValidate : function(){
      ...
      $.jvalidator.addPattern({
        ...
        validate: function(value, validationCallback) {
          var _self = $(this);
          var realCardNo = $.trim(value).replace(/\s/g, '');
          var isMasterCard = me.isValidMasterCard(realCardNo);

          if (isMasterCard) {
            me.showCreditCardInfo( realCardNo, _self, function(res){ //callback 时添加正确标示
      ...
      this.formChecker= $('form').find("[data-jvalidator-pattern]").jvalidator({
        ...
        on: {
          invalid: function(evt,el,patterns) {
            ...
            if($(el).is("#credit_cardnum")){
              e.find(".icon_tips_wrap").addClass("first");
            }
          },
...
;(function(){
	...
	var clickStatic = function( option ){
		...
		optExt.timeStamp = +new Date();
		optExt.order = $(".order_num").text();
		optExt.uid = !!QNR && !!QNR.PayInfo ?  QNR.PayInfo.userid || "" : "";

		$.extend(option,optExt);

		GA.clk( option );
	}

	// tab
	$(".tab_nav a").live("click",function(){
		var index = $(this).index(),
			name = ['credit','debit','platform'];
		clickStatic({a:"tab_"+name[index]});
	});

	//订单详情
	$(".detail_more_ctl .show").click(function(){
		clickStatic({a:"orderDetail"});
	});

	// next
	$("#btnSubmitDebitCard_next").click(function(){
		clickStatic({a:"debitCartNext"});
	});
	$("#btnSubmitPlatform_next").click(function(){
		clickStatic({a:"platformNext"});
	});

	// submit
	$("#btnSubmit").live("click",function(){
		var opt = {a:"creditSubmit"};
		// opt.cardnum = $("#credit_cardnum").val();
		// opt.youxiao = $(".yselector_input").eq(0).text().replace(/[^\d]/g,"") + "-" + $(".yselector_input").eq(1).text().replace(/[^\d]/g,"");
		// opt.cardbacknum = $("#credit_cardbacknum").val();
	...
	window.onbeforeunload = function(){
		clickStatic( {a:"windowClose"} );
	};

	// 协议
	$(".protoInfo").live("click",function(){
		clickStatic( {a:"xieyi"} );
	});

	// 选中协议
	$("input[name=isBind]").change(function(){
		var checked = $(this).is(":checked");
		clickStatic( {a:"xieyi_"+(checked ? "checked" : "unchecked" )} );
	});

	// 展开全部储蓄卡
	$("#show_and_hide .show").click(function(){
		clickStatic( {a:"openAll"} );
	});

	// 添加其他信用卡
	$("#choosebank").live("click",function(){
		clickStatic( {a:"chooseOther"} );
	});
