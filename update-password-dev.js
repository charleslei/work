...
(function(ns){
	...
	_V.prototype={
		...
		checkSubmit: function(el,r){
			...
			//console.log("processing",el,r,this.processing,this.submitLock);
			...
			this.dom.bind("submit",$.proxy(function(e){
				...
				for( var i=0; i<data.length; i++){
					...
					if(data[i].v.sync){
						...
						r=data[i].valid();
						//console.log(r,data[i].v.name);
						if(!r){
							this.submitLock++;
							//console.log(data[i].v.name,"lock"+this.submitLock);
							data[i].func();
							//console.log(data[i].v.name,"then lock"+this.submitLock);
						}else if(data[i].v.require&&(r=="empty"||r=="tip")){
							this.update(data[i].el,data[i].v,"require");
							err++;
						}
				...
				if(this.submitLock){
					//console.log("提交失败, 因为异步验证未通过或等待异步验证中");
					return false;
				}
...
$(function(){
	var form=$("#update-pwd"), tips=QNR.locale.tips;
	
	var status=form.attr("status");
	if(status){
		//NEWPWDERROR		:	新交易密码错误
		//NEWPWD2ERROR		:	新交易密码重复输入错误
		//NEWPWDSAMEERROR	:	新交易密码与用户中心密码一致
		//LOCK				:	交易密码锁定！
		//FORBIDMODIFY		:	修改次数超限！
		//OLDPWDERROR		:	原密码输入错误！
		//OLDPWDERRORONLYONECE:	原密码还剩一次尝试机会
		//OLDPWDERRORLOCK	:	原密码被锁
		//SUCCESS
		//FAIL
		...
		switch(status){
			...
			case "FORBIDMODIFY":
				...
				$("#page-title").append($('<span class="open_errorstip" placeholder="tip"><span class="icon_error"></span>'+tips.updatepwd[status]+'</span>'));
				return;
		}
	}
	
	$('[js-valid-type="oldcode"],[js-valid-type="oldcode"],[js-valid-type="oldcode"]').bind("focus",function(){
		$('#page-title [placeholder="tip"]').hide();
	});
	
	var robot=new QNR.formRobot({
		form: form,
		patterns: {
			"securepwd":function(str){
				var r=false;
				r=/^[\x00-\xff]{6,16}$/.test(str);
				...
				//判断如果 全是字符s==l 或者 全是数字n==l, 或者字符和数字数量都是0, 也就是全是符号
				if(s==l||n==l||(!s&&!n)){
					return false;
				}
		...
		validators: {
			"oldcode": {
				tip: tips.oldpwd.tip,
				error: tips.oldpwd.error,
				ok: false,
				events: ["blur","submit"],
				require: tips.oldpwd.require,
				pattern: function(str){
					if($.trim(str)==''){
						return "empty";
					}
				
					if(robot.valid("securepwd",str)){
						robot.find("newcode").trigger("check");
						return true;
					}
			...
			"newcode": {
				tip: tips.securepwd.tip,
				error: tips.securepwd.error,
				ok: "",
				sync: false,
				events: ["blur","submit","check"],
				require: tips.securepwd.require,
				trigger: "password-blur",
				callback: "password-check",
				pattern: "securepwd"
			},
			"repeatcode": {
				tip: tips.repeatpwd.tip,
				error: tips.repeatpwd.error,
				ok: "",
				events: ["blur","submit","check"],
				require: tips.repeatpwd.require,
				pattern: function(str){
					...
					return str==robot.find("newcode").val();
				}
			}
		},
		
		submit: function(){
			$('#page-title [placeholder="tip"]').remove();
			
			$.ajax({
				url: '../security/modifyTradePwd.htm',
				dataType: "json",
				data: {
					oldPwd: robot.find("oldcode").val(),
					newPwd: robot.find("newcode").val(),
					newPwd2: robot.find("repeatcode").val()
				},
				type: "post",
				success: function(data){
					
					robot.getValidator("oldcode").removeTips();
					robot.getValidator("newcode").removeTips();
					robot.getValidator("repeatcode").removeTips();
					if(data.ret){
						form[0].reset();
						$("#page-title").append($('<span class="open_sucstip" placeholder="tip"><span class="icon_correct2"></span>'+tips.updatepwd['SUCCESS']+'</span>'));
					}else{
						switch(data.errcode){
							case "OLDPWDERROR":
								robot.getValidator("oldcode").showError(tips.password.error);
								break;
							case "OLDPWDERRORONLYONECE":
								robot.getValidator("oldcode").showError(tips.password.limit);
								break;
							case "OLDPWDERRORLOCK":
								robot.getValidator("oldcode").showError(tips.password._locked);
								break;
							case "LOCK":
							case "FORBIDMODIFY":
								form[0].reset();
								form.find("input,button").attr("disabled",true);
							default:
								$("#page-title").append($('<span class="open_errorstip" placeholder="tip"><span class="icon_error"></span>'+tips.updatepwd[data.errcode]+'</span>'));
						}
					}
				}
			});
	...
	var pwd=robot.find("newcode");
	pwd.bind("password-blur",function(e,str,_config){
		if(!robot.valid("securepwd",pwd.val())){
			pwd.trigger("password-check",[false]);
			return false;
		}
		
		if(pwd.val()==robot.find("oldcode").val()){
			pwd.trigger("password-check",[false]);
			robot.getValidator("newcode").showError(tips.updatepwd.repeat);
			return false;
		}
		
		robot.getValidator("newcode").removeTips();
		
		var url="../security/validateTradePwd.htm";
		...
		$.ajax({
			...
			success:function(data){
				...
				if(!data.ret){
					...
					if(data.errmsg){
						pwd.trigger("password-check",[false]);
						robot.getValidator("newcode").showError();
					}
					
					return;
				}
				
				//dialog的返回status， 非dialog的用success
				if(parseInt(data.data.success,10)){
					pwd.trigger("password-check",[true]);
					robot.find("repeatpwd").trigger("check");
				}else{
					pwd.trigger("password-check",[false]);
					robot.getValidator("newcode").showError(tips.securepwd.repeat);
				}
			}
		});
	});
	
	/*<span class="open_sucstip"><span class="icon_correct2"></span>修改次数超限！</span>*/
});
/**
...
QNR.StepByStep=function(id, id1){
  obj = $('#' + id);
  obj1 = $('#' + id1);

  var step = function(title, con) {
    this.title = title;
    this.step0 = {'0': '选择方式', '1': '确认原手机信息', '2': '新手机验证', '3': '修改成功'};
  ...
  step.prototype = {
    changeStep: function(dir){
      ...
      if(this.stepNum == 0){
        ...
        $(this.currentCon[this.stepNum - 1]).show();
      }
    },
    changeDesc: function  (i) {
      if(i == '0'){
        this.currentDesc = this.step0;
        this.currentCon = this.con0;
      }else{
        this.currentDesc = this.step1;
        this.currentCon = this.con1;
      }
      var spans = this.title.children('span');
      $(spans[1]).text(this.currentDesc[1]);
      $(spans[2]).text(this.currentDesc[2]);
    }
  }

  return new step(obj, obj1);
}
